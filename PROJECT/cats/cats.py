...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if close_word is not None and min_diff <= limit:
        return close_word
    else:
        return typed_word
    # END PROBLEM 5
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if i < len(source) and typed[i] == source[i]:
            ...
            break
    progress = count / len(source) if len(source) > 0 else 0.
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[t2 - t1 for t1,t2 in zip(ts,ts[1:])] for ts in tpp]
    words = words[:len(tpp[0]) - 1]  # The words typed by each player
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for j in word_indices:
        times_for_word = [get_time(times, i, j) for i in player_indices]
        best_time = min(times_for_word)
        winner = times_for_word.index(best_time)
        fastest[winner].append(words[j])
    return fastest
