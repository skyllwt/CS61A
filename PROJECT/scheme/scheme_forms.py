...
def make_let_frame(bindings, env):
    ...
    while current is not nil:
        ...
        if not scheme_symbolp(binding.first):
            raise SchemeError('bad binding in let form: {}'.format(binding.first))
        temp_names.append(binding.first)
        temp_vals.append(scheme_eval(binding.rest.first, env))
        current = current.rest
        
    for name_time  in reversed(temp_names):
        names = Pair(name_time,names)
    for val_time in reversed(temp_vals):
        vals = Pair(val_time, vals)
