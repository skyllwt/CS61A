...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        operand_values = rest.map(lambda x:scheme_eval(x, env) )
        return scheme_apply(operator_procedure, operand_values,env)
        # END PROBLEM 3
...
def eval_all(expressions, env):
    ...
    last_value = None
    while expressions_list is not nil:
        current_value = scheme_eval(expressions_list.first, env)
        last_value = current_value
        expressions_list = expressions_list.rest
    return last_value
    # END PROBLEM 6
